...
export default function LeadToast() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 28, right: 28, zIndex: 9999, width: 320, background: '#1c1917', borderRadius: 'var(--radius)', boxShadow: '0 8px 32px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '14px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <button onClick={dismissToast} style={{ color: '#78716c', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1rem' }}>×</button>
        </div>
        <div style={{ fontSize: '0.9rem', fontWeight: 500, color: '#fff', marginBottom: 3 }}>{toast.name}</div>
        <div style={{ fontSize: '0.78rem', color: '#a8a29e', marginBottom: 12 }}>{toast.contact}{toast.service_detail && <> · {toast.service_detail.name}</>}</div>
