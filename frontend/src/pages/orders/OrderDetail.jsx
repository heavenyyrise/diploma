...
export default function OrderDetail() {
  ...
  useEffect(() => {
    ordersApi.get(id).then(r => {
      ...
      // Load full client info if client exists
  ...
  const save = async () => {
    ...
    try {
      ...
      setOrder(r.data)
      setEditing(false)
      // Refresh client detail if changed
      if (r.data.client_detail?.id) {
        clientsApi.get(r.data.client_detail.id).then(cr => setClientDetail(cr.data))
      } else {
  ...
  // Use clientDetail if loaded, otherwise fall back to client_detail from order
    ...
    <div style={{ padding: '36px 40px', maxWidth: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Услуги в режиме редактирования */}
          ...
          {/* Контакты клиента (только просмотр, не редактируется здесь) */}
          {client && !editing && (
            <Card style={{ padding: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {clientDetail.contacts.map(c => (
                    <div key={c.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '0.875rem' }}>{c.value}</span>
                    </div>
                  ))}
                </div>
              ) : (
