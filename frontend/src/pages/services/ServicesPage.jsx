...
function ServiceEditInline({ service, onUpdated }) {
  ...
  const save = async () => { setSaving(true); try { await servicesApi.update(service.id, form); onUpdated(); setEditing(false) } finally { setSaving(false) } }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, alignItems: 'end' }}>
        <Field label="Название"><input style={inputStyle} value={form.name} onChange={e => set('name', e.target.value)} /></Field>
      ...
      <div style={{ display: 'flex', gap: 8 }}><Button size="sm" variant="ghost" onClick={() => setEditing(false)}>Отмена</Button><Button size="sm" onClick={save} disabled={saving}>{saving ? '...' : 'Сохранить'}</Button></div>
    </div>
  )
}
